...
A transient Firestore failure - briefly unreachable, a deadline, a quota - renders an "unavailable" line in place of the panel, so a blip costs this section and not the rest of the page. Everything else is rethrown, a missing database included, and no error boundary wraps the section, so a misconfiguration takes the page down where you cannot miss it. That is deliberate: a setup error hiding behind a permanent soothing message is worse than one that fails loudly.
